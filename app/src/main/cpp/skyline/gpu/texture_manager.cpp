...
namespace skyline::gpu {
    ...
    void TextureManager::CopyToTexture(RecordFunction recordCb, const std::shared_ptr<Texture> &source, const std::shared_ptr<Texture> &destination, u32 sourceLevel, u32 sourceLayer, u32 destinationLevel, u32 destinationLayer, u32 levelCount, u32 layerCount) {
        ...
        if (aspect == vk::ImageAspectFlags{}) {
            Logger::Warn("Source and destination textures have no common aspect: {} -> {}", vk::to_string(sourceHost->format->vkFormat), vk::to_string(destinationHost->format->vkFormat));
            return;
        } else if (sourceHost->dimensions != destinationHost->dimensions) {
            Logger::Warn("Source and destination textures have different dimensions: {}x{}x{} -> {}x{}x{}", sourceHost->dimensions.width, sourceHost->dimensions.height, sourceHost->dimensions.depth, destinationHost->dimensions.width, destinationHost->dimensions.height, destinationHost->dimensions.depth);
            return;
        } else if (sourceHost->sampleCount != destinationHost->sampleCount) {
            Logger::Warn("Source and destination textures have different sample counts: {} -> {}", vk::to_string(sourceHost->sampleCount), vk::to_string(destinationHost->sampleCount));
            return;
        } else if (sourceHost->layout == vk::ImageLayout::eUndefined) {
            Logger::Warn("Source texture has undefined layout");
            return;
        }
